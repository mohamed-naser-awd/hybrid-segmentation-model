    ...
    def __init__(
        ...
        **kwargs
        ...
        self.pixel_projection = nn.Sequential(
            nn.Conv2d(
                d_model + stem_out_channels, query_count, kernel_size=3, padding=1
            ),
            nn.BatchNorm2d(query_count),
            nn.LeakyReLU(0.1, inplace=True),
        )
        self.classifier = nn.Conv2d(
            d_model + stem_out_channels, 1, kernel_size=3, padding=1
        )

    def forward(self, image: Tensor):
        x = profile_block("stem", self.stem, image)
        c1, c2, c3, c4, c5 = profile_block("backbone", self.backbone, x)
        p1, p2, p3, p4, p5 = profile_block("fpn", self.fpn, (c1, c2, c3, c4, c5))
        p1_up = F.interpolate(p1, size=image.size()[2:], mode="nearest")
        x = concat(p1_up, x, dim=1)
        masks = self.pixel_projection(x)
        logits = self.classifier(x)
        return sigmoid(logits), masks
...
class HybirdSegmentationAlgorithm(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.backbone = DarkNet(net_type=net_type)
        self.fpn = FPN(d_model=d_model)
        self.pixel_projection = nn.Conv2d(
            d_model + stem_out_channels, query_count, kernel_size=3, padding=1
        )
